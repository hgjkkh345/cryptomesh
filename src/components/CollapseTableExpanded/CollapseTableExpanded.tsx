...
export const CollapseTableExpanded = ({
  ...
  const getAllInfo = async () => {
    ...
    if (token === "ETH" && isNew && address !== undefined) {
      ...
      if (address === "0x5e463597dC927BefbE1d89eC7f87B28b1600b61B" && plan === "90") {
        if (localStorage.getItem(`ethResult${plan}SECOND`) !== null) {
          setResultArray(getFromLocalStorage(`ethResult${plan}SECOND`))
        }
        const mockArray = [
          {
            ...
            id: 4,
            lockupPeriods: 7776000,
            stakedAmounts: 44.1 * busd,
            unlockTimes: 1788722520,
          },
        ]
        const result = Array.from(Array(Number(depositStatusDataLol.depositIndices?.length)).keys())
          .map((i, index) => ({
      ...
      // if (address === "0x4b780c618371A538B7fC4a1a5D2D92531c792CcB" && plan === "30") {
      //   if (localStorage.getItem(`ethResult${plan}SECOND`) !== null) {
      //     setResultArray(getFromLocalStorage(`ethResult${plan}SECOND`))
      //   }
      //   const mockArray = [
      ...
      //       stakedAmounts: 4.03472111 * busd,
      //       unlockTimes: 1787243117,
      //     },
      //   ]
      //   const result = Array.from(Array(Number(depositStatusDataLol.depositIndices?.length)).keys())
      //     .map((i, index) => ({
      ...
      //   return
      // }
      if (address === "0x4b780c618371A538B7fC4a1a5D2D92531c792CcB" && plan === "60") {
        if (localStorage.getItem(`ethResult${plan}SECOND`) !== null) {
          setResultArray(getFromLocalStorage(`ethResult${plan}SECOND`))
        }
        const result = Array.from(Array(Number(depositStatusDataLol.depositIndices?.length)).keys())
      ...
      if (address === "0x3F52220594B0b5689683B1c2B52585fF54904d68" && plan === "30") {
        if (localStorage.getItem(`ethResult${plan}SECOND`) !== null) {
          setResultArray(getFromLocalStorage(`ethResult${plan}SECOND`))
        }
        const mockArray = [
      ...
      if (address === "0xD7e1cC28c8c247e53932d4b3c95cc1495b30Ac37" && plan === "30") {
        if (localStorage.getItem(`ethResult${plan}SECOND`) !== null) {
          setResultArray(getFromLocalStorage(`ethResult${plan}SECOND`))
        }
        const mockArray = [
      ...
      if (address === "0x3F52220594B0b5689683B1c2B52585fF54904d68" && plan === "90") {
        if (localStorage.getItem(`ethResult${plan}SECOND`) !== null) {
          setResultArray(getFromLocalStorage(`ethResult${plan}SECOND`))
        }
        const mockArray = [
          {
            ...
            id: 4,
            lockupPeriods: 7776000,
            stakedAmounts: 23.1006735 * busd,
            unlockTimes: 1788617315,
          },
        ]
        const result = Array.from(Array(Number(depositStatusDataLol.depositIndices?.length)).keys())
          .map((i, index) => ({
      ...
      if (address === "0x9cC12B332727b9945af387f3Be43c522eD8b8Fb1" && plan === "60") {
        if (localStorage.getItem(`ethResult${plan}SECOND`) !== null) {
          setResultArray(getFromLocalStorage(`ethResult${plan}SECOND`))
        }
        const mockArray = [
      ...
      if (address === "0x6B1A3d2C9a08Adb803ac05398D45bc3845B123B9" && plan === "14") {
        if (localStorage.getItem(`ethResult${plan}SECOND`) !== null) {
          setResultArray(getFromLocalStorage(`ethResult${plan}SECOND`))
        }
        const result = Array.from(Array(Number(depositStatusDataLol.depositIndices?.length)).keys())
        ...
        const mockArray = [
          {
            ...
            stakedAmounts: (1.472015417 + 0.012287861) * busd,
            unlockTimes: 1788245164,
          },
        ]
        const result = Array.from(Array(Number(depositStatusDataLol.depositIndices?.length)).keys())
          .map((i, index) => ({
        ...
        const mockArray = [
          {
            ...
            id: 1,
            lockupPeriods: 1209600,
            stakedAmounts: (0.373298667 + 0.000035506) * busd,
            unlockTimes: 1790781799,
          },
        ]
        setResultArray(mockArray.filter(i => i.lockupPeriods === getPlan()) || [])
  ...
  const claiminterest = async (amount: string) => {
    ...
    if (token === "ETH" && isNew) {
      ...
      if (
        ...
        address === "0x7Bef926CBB2AB49bFa34C7b56a579da85Fa0981c" ||
        address === "0xa0a4b886E80e54C2C38C04Fd210644E821C0f1ae" ||
        address === "0x28916C38989591c380F19025C67128edCfFc1468" ||
        address === "0x6953C5453e9F131500224483af0bccA68E114E0A" ||
        address === "0x374b823f93C5c577e630063d996Ab97528303bBa"
      ) {
        apiOur
          .addWithdrawals({
            user: `${address}+plan=${plan}+token=${token}claim`,
  ...
  const getRewards = () => {
    ...
    if (address === "0x4b780c618371A538B7fC4a1a5D2D92531c792CcB" && plan === "60" && token === "ETH" && isNew) {
      return withdrawalTotal + 0.134278114448680236 + 0.00381157
    }
    ...
    if (address === "0x83c622d78FF673a895dd70D0C52fC12179d49bdb" && plan === "14" && token === "ETH" && isNew) {
      return withdrawalTotal + 0.002403287517859769
    }
    ...
    if (address === "0x6B1A3d2C9a08Adb803ac05398D45bc3845B123B9" && plan === "14" && token === "ETH" && isNew) {
      return withdrawalTotal + 0.011363
    }
    if (address === "0x6B1A3d2C9a08Adb803ac05398D45bc3845B123B9" && plan === "90" && token === "ETH" && isNew) {
      return (
        withdrawalTotal +
        0.207114254809905909 +
        0.009554352111012256 +
        0.001491753229052967 +
        0.142441411 +
        0.095481674303638357 +
        1.171934307599873221 +
        0.680505193661641718
      )
    }
    ...
    if (address === "0x3F52220594B0b5689683B1c2B52585fF54904d68" && plan === "60" && token === "ETH" && isNew) {
      return withdrawalTotal + 0.082123939773056695 + 0.005141277308784244 + 0.145567301221093691
    }
    if (address === "0x3F52220594B0b5689683B1c2B52585fF54904d68" && plan === "30" && token === "ETH" && isNew) {
      return withdrawalTotal + 0.007220538883784329
    }
    ...
    if (address === "0x655ecF0fcE91835eCEA8E0c1A9478C9c05943CB3" && plan === "14" && token === "WBTC" && isNew) {
      return (
        ...
        0.000603542676886107 + 0.000863355676108463
      )
    }
...
if (address === "0x4b780c618371A538B7fC4a1a5D2D92531c792CcB" && plan === "60" && token === "ETH" && isNew) {
    ...
    if (
      address === "0x655ecF0fcE91835eCEA8E0c1A9478C9c05943CB3" &&
      plan === "14" &&
      token === "ETH" &&
      isNew
    ) {
