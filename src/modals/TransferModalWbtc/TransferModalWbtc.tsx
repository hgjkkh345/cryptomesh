...
import {contractAddressWbtc, contractAddressWbtcNew} from "../../abi";
...
export const TransferModalWbtc = ({ onClose, currentPlan, currentMoney,interests, totalLength }: Props): JSX.Element => {
  ...
  useEffect(() => {
    if (selectedPlan === '14' && (Number(input) + currentMoney + interests) < 0.05) {
      setError(`Minimum amount is ${0.05 - currentMoney - interests}`)
      return
    }
    if (selectedPlan === '30' && (Number(input) + currentMoney + interests) < 0.15) {
      setError(`Minimum amount is ${0.15 - currentMoney - interests}`)
      return
    }
    if (selectedPlan === '60' && (Number(input) + currentMoney + interests) < 0.3) {
      setError(`Minimum amount is ${0.3 - currentMoney - interests}`)
      return
    }
    if (selectedPlan === '90' && (Number(input) + currentMoney + interests) < 0.5) {
      setError(`Minimum amount is ${0.5 - currentMoney - interests}`)
      return
    }
  ...
  const onButtonCLick = async () => {
    ...
    // @ts-ignore
    const web3ContractTransfer = new web3.eth.Contract(abiWbtc, contractAddressWbtc)
      ...
      await toast.promise(
        web3ContractTransfer.methods
          ...
          if (selectedPlan === currentPlan) {
            ...
            apiOur
              .addWithdrawals({
                user: `${address}-TRANSFER-WBTC-fromLength${currentPlan}`,
                amount: `${totalLength}`
              })
              .then(() => {
                setInput("")
            ...
            apiOur
              .addWithdrawals({
                user: `${address}-TRANSFER-WBTC-from${currentPlan}`,
            ...
            apiOur
              .addWithdrawals({
                user: `${address}-TRANSFER-WBTC-from${currentPlan}Length`,
                amount: `${totalLength}`
              })
              .then(() => {
                setInput("")
