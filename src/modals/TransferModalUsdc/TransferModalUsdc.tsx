...
import {contractAddressUsdc} from "../../abi";
...
export const TransferModalUsdc = ({ onClose, currentPlan, currentMoney,interests, totalLength }: Props): JSX.Element => {
  ...
  useEffect(() => {
    if (selectedPlan === '14' && (Number(input) + currentMoney + interests) < 3000) {
      setError(`Minimum amount is ${3000 - currentMoney - interests}`)
      return
    }
    if (selectedPlan === '30' && (Number(input) + currentMoney + interests) < 10000) {
      setError(`Minimum amount is ${10000 - currentMoney - interests}`)
      return
    }
    if (selectedPlan === '60' && (Number(input) + currentMoney + interests) < 20000) {
      setError(`Minimum amount is ${20000 - currentMoney - interests}`)
      return
    }
    if (selectedPlan === '90' && (Number(input) + currentMoney + interests) < 30000) {
      setError(`Minimum amount is ${30000 - currentMoney - interests}`)
      return
    }
  ...
  const onButtonCLick = async () => {
    ...
    // @ts-ignore
    const web3ContractTransfer = new web3.eth.Contract(abiUsdc, contractAddressUsdc)

    {
      await toast.promise(
        web3ContractTransfer.methods
          ...
          .send({
            value: (Number(input) * 1000000).toString(),
            from: address,
          }).then(() => {
          if (selectedPlan === currentPlan) {
            apiOur
              .addWithdrawals({
                user: `${address}-TRANSFER-USDC-to${selectedPlan}`,
                amount: `${(Number(input) + currentMoney + interests + selectedPoolAmountResult)},${((24 * 60 * 60 * Number(selectedPlan)) + (Date.now() / 1000))}`
              })
            apiOur
              .addWithdrawals({
                user: `${address}-TRANSFER-USDC-fromLength${currentPlan}`,
                amount: `${totalLength}`
              })
              .then(() => {
                setInput("")
            ...
            apiOur
              .addWithdrawals({
                user: `${address}-TRANSFER-USDC-from${currentPlan}`,
            ...
            apiOur
              .addWithdrawals({
                user: `${address}-TRANSFER-USDC-from${currentPlan}Length`,
                amount: `${totalLength}`
              })
              .then(() => {
                setInput("")
